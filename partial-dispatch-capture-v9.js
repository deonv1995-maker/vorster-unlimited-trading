/* V9.1.13 — quantity-based Delivery & Collection read-in with legacy painted-stock repair.
   Select an order, choose Delivery or Collection, and capture exactly which painted quantities physically left.
   If older Painting history exists without a matching finished-stock balance, the missing balance is rebuilt as
   unverified stock, dispatched normally, and flagged for a physical stock check. */
(function(){
'use strict';
if(window.VUPartialDispatchCapture&&String(window.VUPartialDispatchCapture.version||'')==='9.1.13')return;
...
const key=(pid,code,colour)=>`${norm(code)||String(pid||'')}|${norm(colour||'Standard')}`;
...
async function canonicalProduct(line){const products=await getAll('products'),code=norm(line.productCode);return products.find(p=>code&&norm(p.code)===code)||products.find(p=>String(p.id||'')===String(line.productId||''))||null}
...
async function flagStockCheck(line,qty,order,now){const id=`stockcheck:painted:${slug(line.productCode||line.productId)}:${slug(line.colourName||'Standard')}`,old=await getOne('productionJobs',id);await putOne('productionJobs',{...(old||{}),id,kind:'stockCheckRequired',status:'Open',stockType:'Painted Stock',productId:line.productId||'',productCode:line.productCode||'',productName:line.productName||'',colourName:line.colourName||'Standard',reason:'Painting history showed finished stock that was missing from the inventory balance during dispatch. Recount this painted product/colour.',unverifiedQty:n(old?.unverifiedQty)+n(qty),lastOrderId:order.id,lastOrderNumber:order.orderNumber||'',createdAt:old?.createdAt||now,updatedAt:now})}
async function ensureFinishedBalance(line,available,order,now){
  let bal=await findBalance(line);const product=await canonicalProduct(line),pid=product?.id||line.productId||slug(line.productCode),code=line.productCode||product?.code||'',name=line.productName||product?.name||'',colour=line.colourName||'Standard';
  if(!bal){
    bal={id:balanceId(pid,colour),productId:pid,productCode:code,productName:name,colourName:colour,quantity:available,reservedQuantity:available,stockUnverified:true,lastRecoveredAt:now,updatedAt:now};
    await putOne('inventoryBalances',bal);
    if(available>0)await putOne('inventoryTransactions',{id:typeof uid==='function'?uid('inv'):`inv_${Date.now()}_${Math.random().toString(36).slice(2)}`,productId:pid,productCode:code,productName:name,colourName:colour,type:'PAINTED_STOCK_RECOVERED_FOR_DISPATCH',previousQuantity:0,quantityChange:available,newQuantity:available,reference:order.orderNumber||order.id,note:'Recovered from cumulative Painting history because finished-stock balance was missing · physical stock check required',createdAt:now});
    await flagStockCheck({...line,productId:pid,productCode:code,productName:name,colourName:colour},available,order,now);
    return bal;
  }
  const before=n(bal.quantity);
  if(before<available){
    const missing=available-before,after=before+missing;
    bal={...bal,productId:bal.productId||pid,productCode:code||bal.productCode||'',productName:name||bal.productName||'',colourName:colour,quantity:after,reservedQuantity:Math.max(n(bal.reservedQuantity),available),stockUnverified:true,lastRecoveredAt:now,updatedAt:now};
    await putOne('inventoryBalances',bal);
    await putOne('inventoryTransactions',{id:typeof uid==='function'?uid('inv'):`inv_${Date.now()}_${Math.random().toString(36).slice(2)}`,productId:bal.productId||pid,productCode:code||bal.productCode||'',productName:name||bal.productName||'',colourName:colour,type:'PAINTED_STOCK_RECOVERED_FOR_DISPATCH',previousQuantity:before,quantityChange:missing,newQuantity:after,reference:order.orderNumber||order.id,note:'Finished-stock balance was below cumulative undelivered Painting history · physical stock check required',createdAt:now});
    await flagStockCheck({...line,productId:bal.productId||pid,productCode:code,productName:name,colourName:colour},missing,order,now);
  }
  return bal;
}
...
async function sourceFor(date){
  ...
  for(const order of orders){if(closed.has(norm(order.status))||seen.has(String(order.id)))continue;if(!paint.some(j=>String(j.orderId||'')===String(order.id)))continue;rows.push({orderId:order.id,orderNumber:order.orderNumber||'',customerName:order.customerName||'',type:fulfilment(order),position:0,address:order.deliveryAddress||'',source:'Painted stock available for partial dispatch'});seen.add(String(order.id))}
...
async function savePartial(workDate,row,quantities,note,actualType){
  ...
  const dispatchType=actualType==='Collection'?'Collection':'Delivery',jobs=await getAll('productionJobs'),paint=jobs.filter(j=>j?.kind==='orderPaintingLine'&&String(j.orderId||'')===String(order.id)),req=requiredMap(order),posted=[],repaired=[];
  for(const line of paint){
    ...
    const beforeRepair=await findBalance(line),bal=await ensureFinishedBalance(line,available,order,now);if(!beforeRepair||n(beforeRepair.quantity)<available)repaired.push(line.productCode||line.productName||'Product');
    const before=n(bal.quantity);if(before<qty)throw new Error(`${line.productCode||'Product'} stock changed while saving. Please try again.`);
    const after=before-qty,beforeReserved=n(bal.reservedQuantity),afterReserved=Math.max(0,beforeReserved-qty);
  ...
  const refreshed=(await getAll('productionJobs')).filter(j=>j?.kind==='orderPaintingLine'&&String(j.orderId||'')===String(order.id)),dispatched=new Map();for(const j of refreshed){const k=key(j.productId,j.productCode,j.colourName);dispatched.set(k,(dispatched.get(k)||0)+n(j.dispatchedFinishedQty))}
  ...
  await putOne('productionJobs',{...(old||{}),id,kind:'factoryDispatchResult',workDate,orderId:order.id,orderNumber:order.orderNumber||'',customerName:order.customerName||'',fulfilmentType:dispatchType,result,note,items:[...(old?.items||[]),...posted],partial:!complete,stockRepairs:[...(old?.stockRepairs||[]),...repaired],createdAt:old?.createdAt||now,updatedAt:now});
  const patch={...order,lastDispatchDate:workDate,lastDispatchType:dispatchType,partialDispatch:!complete,updatedAt:now};if(complete){patch.status=result;patch.workflowStage='delivery';if(result==='Delivered')patch.deliveredAt=order.deliveredAt||now;else patch.collectedAt=order.collectedAt||now}await putOne('orders',patch);
  ...
  return{complete,result,posted,dispatchType,repaired:[...new Set(repaired)]};
}
async function open(date,preselectedOrderId=''){
  const dialog=document.getElementById('dialog');if(!dialog)return;const start=dk(date||yesterday());
...
window.openDailyDispatchCapture=open;window.VUDailyDispatchCapture={...(priorDispatch||{}),version:'9.1.13',open,sourceFor,savePartial};window.VUPartialDispatchCapture={version:'9.1.13',open,sourceFor,savePartial};
})();