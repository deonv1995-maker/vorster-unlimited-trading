/* V9.1.09 — quantity-based Delivery & Collection read-in.
   Captures what physically left per product/colour, even when the commercial order remains incomplete.
   Uses the selected day's frozen/live plan, plus open orders that still have painted stock available. */
(function(){
'use strict';
if(window.VUPartialDispatchCapture)return;
...
async function sourceFor(date){
  const d=dk(date||new Date()),base=window.VUDailyDispatchCapture?.sourceFor?await window.VUDailyDispatchCapture.sourceFor(d):{workDate:d,source:'Live data',rows:[]};
...
async function savePartial(workDate,row,quantities,note){
  ...
  for(const line of paint){
    ...
    await putOne('inventoryTransactions',{id:typeof uid==='function'?uid('inv'):`inv_${Date.now()}_${Math.random().toString(36).slice(2)}`,productId:bal.productId||line.productId||'',productCode:line.productCode||bal.productCode||'',productName:line.productName||bal.productName||'',colourName:line.colourName||bal.colourName||'Standard',type:'ORDER_PARTIAL_DISPATCHED',previousQuantity:before,quantityChange:-qty,newQuantity:after,reference:order.orderNumber||order.id,note:`${row.type} quantity captured for ${workDate}${note?` · ${note}`:''}`,createdAt:now});
  ...
  const complete=[...req.entries()].every(([k,q])=>n(dispatched.get(k))>=q),result=complete?(row.type==='Collection'?'Collected':'Delivered'):(row.type==='Collection'?'Partially collected':'Partially delivered');
  const id=`dispatchresult:${workDate}:${order.id}`,old=await getOne('productionJobs',id);
  await putOne('productionJobs',{...(old||{}),id,kind:'factoryDispatchResult',workDate,orderId:order.id,orderNumber:order.orderNumber||'',customerName:order.customerName||'',fulfilmentType:row.type,result,note,items:[...(old?.items||[]),...posted],partial:!complete,createdAt:old?.createdAt||now,updatedAt:now});
  const patch={...order,lastDispatchDate:workDate,partialDispatch:!complete,updatedAt:now};
  ...
  try{window.VUOrderProgress?.invalidate?.();if(typeof buildOptimizedOrderJobs==='function')await buildOptimizedOrderJobs()}catch(e){console.warn('Planner recalc after partial dispatch',e)}
  return{complete,result,posted};
}
async function open(date,preselectedOrderId=''){
  ...
  async function loadOrders(){const source=await sourceFor(dateEl.value),rows=source.rows||[];sel.innerHTML='<option value="">Select order</option>'+rows.map(r=>`<option value="${safe(r.orderId)}" ${String(r.orderId)===String(preselectedOrderId)?'selected':''}>${safe(r.orderNumber||'Order')} · ${safe(r.customerName||'Customer')} · ${safe(r.type)}</option>`).join('');sel.dataset.source=source.source||'';if(preselectedOrderId)await render();}
  async function render(){const source=await sourceFor(dateEl.value),row=(source.rows||[]).find(r=>String(r.orderId)===String(sel.value));if(!row){host.innerHTML='<div class="card"><small class="muted">Select an order.</small></div>';return}const order=await getOne('orders',row.orderId),jobs=(await getAll('productionJobs')).filter(j=>j?.kind==='orderPaintingLine'&&String(j.orderId||'')===String(row.orderId)),req=requiredMap(order);const lines=jobs.map((j,i)=>{const k=key(j.productId,j.productCode,j.colourName),available=Math.max(0,n(j.completedQty)-n(j.dispatchedFinishedQty)),required=n(req.get(k));return `<section class="card" data-dispatch-line data-key="${safe(k)}"><h3>${safe(j.productCode||'')} · ${safe(j.productName||'')}</h3><p class="muted">${safe(j.colourName||'Standard')} · Order ${required} · Painted ${n(j.completedQty)} · Already dispatched ${n(j.dispatchedFinishedQty)} · <b>${available} available now</b></p><label>Qty physically ${row.type==='Collection'?'collected':'delivered'} on ${safe(dateEl.value)}<input type="number" min="0" max="${available}" step="1" inputmode="numeric" value="0" data-qty></label></section>`}).join('');host.innerHTML=`<section class="card"><small>${safe(row.type)} · ${safe(source.source||'')}</small><h3>${safe(row.orderNumber||'Order')} · ${safe(row.customerName||'Customer')}</h3><p class="muted">Enter only what physically left. The order remains open unless cumulative dispatched quantities cover the full order.</p></section>${lines||'<div class="card">No painted stock is currently recorded for this order. Capture Painting first.</div>'}<label>Notes / POD / reason<textarea id="partialDispatchNote" placeholder="Optional POD, delivery note or explanation"></textarea></label><button type="button" class="primary" id="partialDispatchSave" ${lines?'':'disabled'}>Save physical dispatch</button>`;const btn=document.getElementById('partialDispatchSave');if(btn)btn.onclick=async()=>{const q={};host.querySelectorAll('[data-dispatch-line]').forEach(el=>q[el.dataset.key]=n(el.querySelector('[data-qty]').value));const old=btn.textContent;btn.disabled=true;btn.textContent='Saving…';try{const res=await savePartial(dateEl.value,row,q,document.getElementById('partialDispatchNote').value.trim());notify?.(`${row.orderNumber||'Order'} · ${res.result}`);close();try{window.VUNavigationAuthority?.refreshCurrent?.()}catch{}}catch(e){console.error('Partial dispatch save',e);alert(e?.message||'Could not save dispatch.');btn.disabled=false;btn.textContent=old}}}
...
window.VUDailyDispatchCapture={...(window.VUDailyDispatchCapture||{}),version:'9.1.09',open,sourceFor,savePartial};
window.VUPartialDispatchCapture={version:'9.1.09',open,sourceFor,savePartial};
})();