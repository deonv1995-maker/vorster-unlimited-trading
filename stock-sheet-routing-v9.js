/* V9.0.54 — authoritative stock-sheet routing.
   ...
   A stock item may belong to exactly one stock sheet. Multiple saved stock-sheet divisions are treated as setup-required. */
(function(){
'use strict';
...
async function productsForStockSheet(division){
...
async function printStockSheet(division){
  const products=await productsForStockSheet(division),rows=await Promise.all(products.map(async p=>({p,qty:await rawQty(p.id)})));
...
async function openStock(division){
  const products=await productsForStockSheet(division),rows=await Promise.all(products.map(async p=>({p,qty:await rawQty(p.id)})));
  openDialog(`<div class="dialog-head"><div><div class="step-label">${safe(division)} division</div><h2>Stock count</h2></div><button class="close-btn" onclick="closeDialog()">×</button></div><p class="muted">This list follows each product's Stock sheet division. Production worksheet routing is separate.</p><div class="division-toolbar"><input id="stockRouteSearch" class="search" placeholder="Search code or product"><button id="stockRoutePrint" class="ghost" type="button">Print stock sheet</button><button id="stockRouteWork" class="ghost" type="button">Print production worksheet</button></div><div id="stockRouteRows" class="division-stock-list">${rows.length?rows.map(({p,qty})=>`<label class="division-stock-row" data-search="${safe(`${p.code||''} ${p.name||''} ${p.category||''}`.toLowerCase())}"><span><strong>${safe(p.code||'')}</strong><small>${safe(p.name||'')}</small><small>Stock sheet: ${safe(stockSheetDivision(p))}</small></span><input type="number" min="0" step="1" inputmode="numeric" enterkeyhint="next" data-stock-route-id="${safe(p.id)}" data-original="${qty}" value="${qty}"></label>`).join(''):`<div class="empty">No products are assigned to the ${safe(division)} stock sheet.</div>`}</div><button id="stockRouteSave" class="primary" type="button" ${rows.length?'':'disabled'}>Save changed quantities</button>`);
  const inputs=[...document.querySelectorAll('[data-stock-route-id]')],byId=new Map(products.map(p=>[String(p.id),p]));
  ...
  document.getElementById('stockRouteSearch').oninput=e=>{const q=norm(e.target.value);document.querySelectorAll('#stockRouteRows .division-stock-row').forEach(row=>row.style.display=!q||row.dataset.search.includes(q)?'':'none')};
  document.getElementById('stockRoutePrint').onclick=()=>printStockSheet(division);
  document.getElementById('stockRouteWork').onclick=()=>window.opPrint?.('production',new Date().toISOString().slice(0,10),division);
  document.getElementById('stockRouteSave').onclick=async()=>{const changed=inputs.filter(i=>num(i.value)!==num(i.dataset.original));if(!changed.length){notify('No stock quantities changed');return;}for(const input of changed){const p=byId.get(String(input.dataset.stockRouteId));if(!p)continue;await saveRaw(p,num(input.value),division);input.dataset.original=String(num(input.value));input.closest('.division-stock-row')?.classList.remove('stock-row-changed');}if(typeof buildOptimizedOrderJobs==='function')await buildOptimizedOrderJobs();notify(`${changed.length} stock ${changed.length===1?'quantity':'quantities'} saved`);};
  requestAnimationFrame(()=>inputs[0]?.focus({preventScroll:true}));
}
function openPicker(){openDialog(`<div class="dialog-head"><div><div class="step-label">Manufacturing</div><h2>Division sheets</h2></div><button class="close-btn" onclick="closeDialog()">×</button></div><p class="muted">Stock sheets follow Stock sheet division. Production worksheets follow Worksheet division.</p><div class="division-picker">${DIVISIONS.map(d=>`<button class="division-card" type="button" data-stock-route-division="${d}"><strong>${d}</strong><small>Stock count · stock sheet · production worksheet</small></button>`).join('')}</div>`);document.querySelectorAll('[data-stock-route-division]').forEach(b=>b.onclick=()=>openStock(b.dataset.stockRouteDivision));}
...
if(typeof baseProducts==='function'){window.productsPage=async function stockRoutingProductsPage(...args){const result=await baseProducts(...args);bindButton();return result;};try{productsPage=window.productsPage}catch{}}
setTimeout(bindButton,0);
window.VUStockSheetRouting={DIVISIONS,stockSheetDivision,productsForStockSheet,openStock,printStockSheet,openPicker,version:'9.0.54'};
})();