/* Version 9.0.3 — holiday-aware capacity-safe completion schedule adapter. */
(function(){
'use strict';
...
window.buildOrderCompletionSchedule=async function(){
  ...
  const source=(result.days||[]).flatMap(day=>(day.items||[]).map(item=>({...item,sourceDate:day.date}))).sort((a,b)=>a.sourceDate.localeCompare(b.sourceDate));
  ...
  for(const item of source){
    const group=`${item.productId}::${norm(item.colourName||'Standard')}`,capacity=Math.max(1,Math.round(Number(item.dailyCapacity||item.quantity||1)));let remaining=Math.max(0,Math.round(Number(item.quantity||0))),date=cal.onOrAfter(item.sourceDate);
    while(remaining>0){
      ...
      const qty=Math.min(space,remaining);used.set(usageKey,already+qty);if(!days.has(date))days.set(date,[]);days.get(date).push({...item,quantity:qty});remaining-=qty;
      const lineKey=`${item.orderId}::${item.productId}::${norm(item.colourName||'Standard')}`;if(!completion.has(lineKey)||date>completion.get(lineKey))completion.set(lineKey,date);
  ...
  for(const plan of result.orders||[]){
    for(const line of plan.linePlans||[]){if(Number(line.toManufacture||0)<=0)continue;const lineKey=`${plan.order.id}::${line.productId}::${norm(line.colourName||line.colour?.name||'Standard')}`;if(completion.has(lineKey))line.completionDate=completion.get(lineKey)}
