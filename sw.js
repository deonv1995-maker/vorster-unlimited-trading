/* V9.3.1 — Digital Factory offline app-shell cache. No forced navigation of open clients. */
const VU_SW_BUILD='9.3.1';
...
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k)));await self.clients.claim();const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});for(const client of clients){try{client.postMessage({type:'VU_BUILD_ACTIVE',build:VU_SW_BUILD})}catch{}}})())});
self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;event.respondWith((async()=>{const cache=await caches.open(CACHE);try{const fresh=await fetch(request,{cache:'no-store'});if(fresh&&fresh.ok&&new URL(request.url).origin===self.location.origin)cache.put(request,fresh.clone()).catch(()=>{});return fresh}catch{const exact=await cache.match(request);if(exact)return exact;if(request.mode==='navigate')return (await cache.match('./index.html'))||(await cache.match('./'));const url=new URL(request.url);return (await cache.match(url.pathname.replace(/^\//,'./')))||Response.error()}})())});