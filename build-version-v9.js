/* V9.2.0 — cleaned operational runtime loader.
   Loads only the final authority for each operational responsibility after the stable base runtime is ready. */
(function(){
'use strict';
const BUILD='V9.2.0';
...
async function loadOperationalExtensions(){
  await waitForBase();
  await loadScriptOnce('production-set-delete-authority-v9.js?v=9.2.0','data-vu-production-set-delete');
  await loadScriptOnce('painting-full-order-authority-v9.js?v=9.2.0','data-vu-paint-full-order');
  await loadScriptOnce('order-update-recalc-authority-v9.js?v=9.2.0','data-vu-order-recalc');
  await loadScriptOnce('worksheet-paint-reconciliation-authority-v9.js?v=9.2.0','data-vu-paint-reconcile');
  await loadScriptOnce('order-identity-reconciliation-v9.js?v=9.2.0','data-vu-order-identity');
  await loadScriptOnce('partial-dispatch-capture-v9.js?v=9.2.0','data-vu-partial-dispatch');
  await loadScriptOnce('dispatch-stepper-authority-v9.js?v=9.2.0','data-vu-dispatch-stepper');
  await loadScriptOnce('live-factory-pack-authority-v9.js?v=9.2.0','data-vu-live-pack');
  applyBuildLabel();
  window.dispatchEvent(new CustomEvent('vu:operational-authorities-ready',{detail:{build:BUILD}}));
}
applyBuildLabel();
loadOperationalExtensions().catch(e=>console.error('V9.2.0 operational authority load failed',e));
window.VUApplyBuildLabel=applyBuildLabel;
window.VUOperationalBuild={version:'9.2.0',ready:()=>String(window.VUPaintingFullOrderAuthority?.version||'')==='9.1.05'&&String(window.VUPartialDispatchCapture?.version||'')==='9.1.14'&&String(window.VUOrderIdentityReconciliation?.version||'')==='9.2.0'&&!!window.VULiveFactoryPackAuthority&&String(window.VUWorksheetPaintReconciliationAuthority?.version||'')==='9.1.12'&&!!window.VUDispatchStepperAuthority&&!!window.VUProductionSetDeleteAuthority&&!!window.VUOrderUpdateRecalcAuthority};
})();