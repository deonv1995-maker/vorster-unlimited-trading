/* Factory OS 2.10.18 — production-safe audited output capture with exact daily-work identity. */
(function(){
'use strict';
...
async function record(item,qty,note='',allowExtra=false){
 ...
 const workKey=String(item?.workKey||item?.key||'').trim()||`${String(item?.orderId||'')}::${String(item?.productId||product.id||'')}::${String(item?.productCode||product.code||'').toUpperCase()}`;
 const workDate=String(item?.workDate||item?.date||window.VUFactoryDailyProductionPlan?.localDate?.()||'').slice(0,10);
 ...
 await window.putOne('inventoryTransactions',{id:window.uid('inv'),productId:product.id,productCode:product.code,productName:product.name,colourName:RAW,type:'PRODUCTION_OUTPUT',division,previousQuantity,quantityChange:quantity,newQuantity,orderId:item.orderId||null,orderNumber:item.orderNumber||null,customerName:item.customerName||null,workKey,workDate,note:String(note||'').trim()||`${division} manufactured output`,recordedByRole:window.VUFactoryOS?.role?.()||'Management',extraStock:quantity>required,createdAt:now});
 return{productId:product.id,division,quantity,previousQuantity,newQuantity,workKey,workDate};
}
async function open(item,onSaved){
 ...
 qty.addEventListener('input',refresh);document.getElementById('fosOutputClose').onclick=window.closeDialog;
...
style();window.VUFactoryProductionOutput={version:'2.10.18',RAW,balanceId,canRecord,rawBalance,record,open};
})();