/* V9.0.53 — strict division stock authority.
   Division stock screens and printed stock sheets use the same exclusive routing as production worksheets:
   worksheetDivision -> primaryDivision -> Unclassified.
   manufacturingMethods and legacy multi-stock visibility are capability/history fields only and do not place a product on multiple sheets. */
(function(){
'use strict';
...
const strictDivision=p=>window.VUStrictDivisionWorksheets?.strictDivision?.(p)||(()=>{const w=String(p?.worksheetDivision||'').trim();if(DIVISIONS.includes(w))return w;const d=String(p?.primaryDivision||'').trim();return DIVISIONS.includes(d)?d:'Unclassified';})();
...
async function strictProducts(division){
  ...
  return products.filter(p=>p.isActive!==false&&strictDivision(p)===division).sort((a,b)=>String(a.code||'').localeCompare(String(b.code||'')));
}
...
async function printStockSheet(division){
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${safe(division)} Stock Count</title><style>${style}</style></head><body><div class="bar"><button onclick="print()">Print / Save PDF</button></div><div class="head"><h1>${safe(division)} Stock Count Sheet</h1><div class="meta">Vorster Unlimited Trading · ${safe(date)} · ${rows.length} products · strict division routing</div></div><table><thead><tr><th>Code</th><th>Product</th><th>Current</th><th>Counted Qty</th></tr></thead><tbody>${body||'<tr><td colspan="4">No products assigned to this division.</td></tr>'}</tbody></table></body></html>`);w.document.close();
}
async function openStock(division){
  const products=await strictProducts(division),rows=await Promise.all(products.map(async p=>({p,qty:await rawQty(p.id)})));
  openDialog(`<div class="dialog-head"><div><div class="step-label">${safe(division)} division</div><h2>Stock count</h2></div><button class="close-btn" onclick="closeDialog()">×</button></div><p class="muted">Only products assigned to ${safe(division)} by Worksheet division, or Primary division when Worksheet division is blank, appear here.</p><div class="division-toolbar"><input id="strictDivisionSearch" class="search" placeholder="Search code or product"><button id="strictDivisionPrint" class="ghost" type="button">Print stock sheet</button><button id="strictDivisionWork" class="ghost" type="button">Print production worksheet</button></div><div id="strictDivisionRows" class="division-stock-list">${rows.length?rows.map(({p,qty})=>`<label class="division-stock-row" data-search="${safe(`${p.code||''} ${p.name||''} ${p.category||''}`.toLowerCase())}"><span><strong>${safe(p.code||'')}</strong><small>${safe(p.name||'')}</small><small>${safe(strictDivision(p))}</small></span><input type="number" min="0" step="1" inputmode="numeric" enterkeyhint="next" data-strict-stock-id="${safe(p.id)}" data-original="${qty}" value="${qty}"></label>`).join(''):`<div class="empty">No products are assigned to ${safe(division)} yet.</div>`}</div><button id="strictDivisionSave" class="primary" type="button" ${rows.length?'':'disabled'}>Save changed quantities</button>`);
...
function openPicker(){openDialog(`<div class="dialog-head"><div><div class="step-label">Manufacturing</div><h2>Division sheets</h2></div><button class="close-btn" onclick="closeDialog()">×</button></div><p class="muted">Each product belongs to one division sheet only. Choose the division you want to count or print.</p><div class="division-picker">${DIVISIONS.map(d=>`<button class="division-card" type="button" data-strict-division="${d}"><strong>${d}</strong><small>Stock count · stock sheet · production worksheet</small></button>`).join('')}</div>`);document.querySelectorAll('[data-strict-division]').forEach(b=>b.onclick=()=>openStock(b.dataset.strictDivision));}
function bindButton(){const b=document.getElementById('divisionSheetsBtn');if(b){b.onclick=openPicker;b.title='Strict division stock and production sheets';}}
...
window.VUStrictDivisionStock={DIVISIONS,strictDivision,strictProducts,openStock,printStockSheet,openPicker,version:'9.0.53'};
})();