(()=>{
  ...
  function close(){const d=document.getElementById('dialog');if(d?.open)d.close();if(d)d.innerHTML=''}
  ...
  async function openMerge(){
    ...
    const options=products.map(product=>`<option value="${esc(product.id)}">${esc(product.code||'')} · ${esc(product.name||'')}</option>`).join('');
    const dialog=document.getElementById('dialog');
    dialog.innerHTML=`<form method="dialog" class="modal-form" style="padding:22px;max-height:88vh;overflow:auto"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><div><div class="eyebrow">PRODUCT DATABASE</div><h2 style="margin:4px 0 0">Merge products</h2></div><button value="cancel" class="icon-btn" aria-label="Close">×</button></div><p class="muted">Choose the duplicate imported product first, then choose the correct existing product to keep.</p><label class="field"><span>Duplicate product to remove</span><select id="mergeSourceNative" class="input"><option value="">Select duplicate product</option>${options}</select></label><label class="field"><span>Correct product to keep</span><select id="mergeTargetNative" class="input"><option value="">Select correct product</option>${options}</select></label><div id="mergeNativePreview" class="card" style="margin-top:16px"><p class="muted">Select two different products.</p></div><button id="mergeNativeConfirm" type="button" class="primary-btn" disabled>Merge products</button></form>`;
    dialog.showModal();
    ...
    const refresh=async()=>{
      const sourceProduct=products.find(product=>product.id===source.value);
      const targetProduct=products.find(product=>product.id===target.value);
      if(!sourceProduct||!targetProduct||sourceProduct.id===targetProduct.id){confirmButton.disabled=true;preview.innerHTML='<p class="muted">Select two different products.</p>';return}
      const orders=await getAll('orders');
      const affectedOrders=orders.filter(order=>(order.lines||[]).some(line=>line.productId===sourceProduct.id)).length;
      preview.innerHTML=`<strong>${esc(sourceProduct.code)} · ${esc(sourceProduct.name)}</strong><p style="margin:8px 0">will be merged into</p><strong>${esc(targetProduct.code)} · ${esc(targetProduct.name)}</strong><p class="muted" style="margin-bottom:0">${affectedOrders} order${affectedOrders===1?'':'s'} currently use the duplicate.</p>`;
      confirmButton.disabled=false;
    };
    source.onchange=refresh;target.onchange=refresh;
    confirmButton.onclick=()=>execute(source.value,target.value);
  }

  async function execute(sourceId,targetId){
    ...
    if(!confirm(`Merge ${source.code} into ${target.code}? This cannot be undone.`))return;
    const now=new Date().toISOString();

    for(const order of orders){
      let changed=false;
      const lines=(order.lines||[]).map(line=>{
        if(line.productId!==source.id)return line;
        changed=true;
    ...
    for(const job of jobs){
      if(job.productId===source.id)await putOne('productionJobs',{...job,productId:target.id,productCode:target.code,productName:target.name,updatedAt:now});
    }

    for(const balance of balances.filter(item=>item.productId===source.id)){
      const colour=balance.colourName||balance.colorName||'Standard';
      const existing=balances.find(item=>item.productId===target.id&&(item.colourName||item.colorName||'Standard')===colour);
    ...
    for(const mapping of mappings.filter(item=>item.type==='product'&&item.targetId===source.id)){
    ...
    if(typeof route==='function')route('products');
    else location.reload();
  }

  window.openNativeProductMerge=openMerge;
  window.addEventListener('DOMContentLoaded',()=>{if(button())button().onclick=openMerge});
})();
