...
const role=()=>localStorage.getItem(ROLE_KEY)||ROLES.MANAGEMENT;
const setRole=value=>{if(!Object.values(ROLES).includes(value))throw new Error('Unknown Factory OS role');localStorage.setItem(ROLE_KEY,value)};
...
window.VUFactoryOS={version:'1.0.0',ROLES,DEFAULTS,role,setRole,settings,saveSettings,snapshot,requirements,dispatchHealth,orderRemainingValue,lineRequired};
})();