/* V9.0.90 — automatic delivery/collection day assignment.
   Algorithm owns the calendar by default. Customer commitments are hard inputs and manager dates are overrides.
   Hard rule: Delivery cannot be operationally scheduled for today when raw-stock coverage is below 75%. */
(function(){
'use strict';
if(window.VUAutoFulfilmentPlanner&&window.VUAutoFulfilmentPlanner.version==='9.0.90')return;
...
async function build(){
  ...
  for(var o=0;o<orders.length;o++){
    var order=orders[o];if(closed[norm(order.status)])continue;var has=false;for(var l=0;l<(order.lines||[]).length;l++)if(num(order.lines[l].qty)>0){has=true;break;}if(!has)continue;
    var sp=scheduleById.get(String(order.id))||{},rank=priority.get(String(order.id))||{},c=customerById.get(String(order.customerId)),com=commitment(order),type=typeOf(order),p=progress.get(String(order.id))||{};
    var earliest=sp.readyNow?today:dk(sp.completionDate);if(!earliest&&num(p.percent)>=100)earliest=today;if(earliest)earliest=workday(earliest);
    var rawPct=Math.max(0,Math.min(100,num(p.rawPct)));
    if(type==='Delivery'&&earliest===today&&rawPct<75)earliest=addWorkdays(today,1);
    active.push({order:order,type:type,commitment:com,customer:c,area:areaOf(order,c),value:num(order.grandTotal),priority:rank.priority||9999,score:num(rank.score),progress:p,rawPct:rawPct,schedule:sp,earliest:earliest,manager:dk(order.managerPlannedDate),savedRoute:savedRouteByOrder.get(String(order.id))||'',due:dk(order.dueDate)});
  }
  ...
  function place(row,k,source,hard,risk){var d=dayState(k);d.value+=row.value;if(row.type==='Delivery')d.deliveries++;else d.collections++;d.areas[row.area]=(d.areas[row.area]||0)+1;d.orders.push(String(row.order.id));var a={order:row.order,date:k,type:row.type,source:source,hard:!!hard,risk:risk||'',priority:row.priority,value:row.value,area:row.area,progress:row.progress,rawPct:row.rawPct,earliest:row.earliest,commitment:row.commitment,schedule:row.schedule};assignments.push(a);return a;}
  for(var x=0;x<active.length;x++){
    var row=active[x],com=row.commitment,sameDayRawBlocked=row.type==='Delivery'&&row.rawPct<75;
    if(com.hard&&dk(com.date)){
      var hardDate=dk(com.date),risk='';
      if(row.earliest&&row.earliest>hardDate)risk='Projected completion is after the customer-required date';
      if(hardDate===today&&sameDayRawBlocked){hardDate=addWorkdays(today,1);risk='Customer requested delivery today, but raw-stock coverage is '+Math.round(row.rawPct)+'%. Same-day delivery is blocked below 75%, so this commitment is at risk.';}
      place(row,hardDate,com.source,true,risk);continue;
    }
    if(row.manager){
      var managerDate=row.manager,mrisk='';if(row.earliest&&row.earliest>managerDate)mrisk='Manager override is earlier than projected completion';
      if(managerDate===today&&sameDayRawBlocked){managerDate=addWorkdays(today,1);mrisk='Manager override cannot bypass the 75% raw-stock rule. Current raw coverage is '+Math.round(row.rawPct)+'%.';}
      place(row,managerDate,'Manager override',true,mrisk);continue;
    }
    if(!row.earliest){unassigned.push({order:row.order,type:row.type,source:'Needs production/setup information',priority:row.priority,value:row.value,progress:row.progress,rawPct:row.rawPct,schedule:row.schedule});continue;}
    ...
    for(var off=0;off<=maxLook;off++){
      var cand=addWorkdays(row.earliest,off);if(cand===today&&row.rawPct<75)continue;
      var state=dayState(cand),score=0,areaCount=state.areas[row.area]||0;score+=areaCount*18;
      ...
      if(row.priority<=5)score-=off*2;if(score>bestScore){bestScore=score;best=cand;}
    }
    var chosen=best||row.earliest,riskNote='';if(chosen!==today&&row.rawPct<75&&row.schedule&&row.schedule.readyNow)riskNote='Same-day delivery blocked because raw-stock coverage is '+Math.round(row.rawPct)+'% (minimum 75%).';
    place(row,chosen,'Algorithm planned',false,riskNote);
  }
  assignments.sort(function(a,b){return a.date.localeCompare(b.date)||a.priority-b.priority;});
  return{generatedAt:new Date().toISOString(),today:today,target:target,assignments:assignments,unassigned:unassigned,days:days,rules:{sameDayDeliveryMinimumRawPercent:75}};
}
