/* V9.3.3 — manager planned dates + explicit manager priority levels influence optimiser priority without replacing customer commitments. */
(function(){
'use strict';
...
const priorityLevel=v=>['high','low'].includes(String(v||'').toLowerCase())?String(v).toLowerCase():'normal';
async function build(){
  const plan=await base.build();
  const rows=(plan.ranked||[]).map(r=>{
    const c=VUOrderCommitment.commitment(r.order),planned=dk(r.order?.managerPlannedDate),days=planned?daysFromToday(planned):999,level=priorityLevel(r.order?.managerPriorityLevel);
    let adjustment=level==='high'?24:level==='low'?-16:0;const reasons=[...(r.reasons||[])];
    if(level==='high')reasons.unshift('manager priority HIGH');
    if(level==='low')reasons.push('manager priority LOW');
    if(planned){
    ...
    return{...r,score:Number((Number(r.score||0)+adjustment).toFixed(2)),managerPriorityLevel:level,managerPlannedDate:planned,managerPlannedType:r.order?.managerPlannedType||VUOrderCommitment.typeOf(r.order),managerPlanDays:days,reasons};
  ...
  for(const r of rows){const protectedDate=(r.commitmentHard&&r.commitmentDays<=7)||(r.managerPlannedDate&&r.managerPlanDays<=5)||r.managerPriorityLevel==='high';if(goal>0&&value>=goal&&r.stage==='production'&&!protectedDate&&r.commitmentDays>7)continue;selected.push(r);value+=Number(r.value||0)}
...
window.VUBusinessOutcomeOptimizer={...base,version:'9.3.3',build,priorityMap};
window.VUManagerPlanPriority={version:'9.3.3'};
})();
