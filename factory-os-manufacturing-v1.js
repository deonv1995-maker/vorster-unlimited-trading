...
async function buildPreviewDivision(division,date=localDate()){
 ...
 return{date,division,effectiveTarget,completed,remaining,assignments,progress:effectiveTarget?Math.min(100,Math.round(completed/effectiveTarget*100)):0};
}
...
function row(a,index){const q=draftQty(a),remaining=n(a.remaining),cap=a.dailyManufacturingCapacity;return `<div class="fos-daily-row" data-work-key="${safe(keyOf(a))}"><div class="fos-daily-info"><div class="fos-priority-tag">TODAY · ${safe(priorityLabel(index))}</div><strong>${safe(a.productCode)} · ${safe(a.productName)}</strong><span>${safe(a.orderNumber)} · ${safe(a.customerName)}</span><span>${safe(a.colour||'Standard')}</span>${a.scheduledDate?`<small>Scheduled ${safe(a.scheduledDate)}</small>`:''}${cap?`<small>Capacity ${Number(cap)} · Moulds ${Number(a.mouldQuantity||0)||'not set'}</small>`:''}</div><div class="fos-daily-required"><b>${remaining}</b><small>remaining today</small></div><div class="fos-daily-stepper ${previewActive()?'is-preview':''}"><button type="button" class="secondary" data-work-minus="${safe(keyOf(a))}" ${previewActive()?'disabled':''}>−</button><input data-work-qty="${safe(keyOf(a))}" type="number" min="0" max="${remaining}" step="1" inputmode="numeric" value="${q}" ${previewActive()?'disabled':''}><button type="button" class="secondary" data-work-plus="${safe(keyOf(a))}" ${previewActive()?'disabled':''}>+</button><button type="button" class="fos-work-all" data-work-all="${safe(keyOf(a))}" ${previewActive()?'disabled':''}>All ${remaining}</button></div></div>`}
...
async function open(requestedDivision){
 ...
 const cleared=daily.remaining===0;
 main.innerHTML=`<section class="card fos-daily-head"><div class="step-label">TODAY’S ${safe(division.toUpperCase())}</div><h2>Today’s ${safe(division)}</h2><p class="muted">${safe(syncText)}</p><div class="fos-daily-summary"><div><span>Completed</span><strong>${daily.completed}</strong></div><div><span>Remaining</span><strong>${daily.remaining}</strong></div></div>${previewActive()?'<p class="muted"><b>Management Preview:</b> quantities are visible but saving is disabled.</p>':''}</section><section class="card"><h2>Today’s work</h2><p class="muted">${visibleAssigned.length} outstanding item${visibleAssigned.length===1?'':'s'} · record only what was physically completed today</p>${visibleAssigned.length?visibleAssigned.map(row).join(''):`<div class="empty">${cleared?'Today’s assigned work is complete.':'No production is required from this division today.'}</div>`}</section>${visibleAssigned.length?`<div class="fos-daily-savebar"><span id="fosDailySelected">0 units selected</span><button class="primary" id="fosDailySave" type="button" disabled>${previewActive()?'Preview only':`Save today’s ${safe(division)}`}</button></div>`:''}`;
